...
int main(void){
  ...
  while(cin >> n){
    ...
    while(n--){
      int x,y;cin >> x >> y;
      pontos.push_back(P(x,y));
    }

    vector<P> polygon = ConvexHull(pontos);
    double sum=0.0;
    int tam = polygon.size();
    for(int i=0;i<tam;i++){
      if(i==tam-1){
        sum += polygon[i].distanceToPoint(polygon[0]);
      }else {
        sum += polygon[i].distanceToPoint(polygon[i+1]);
      }
    }

    cout << setprecision(2) << fixed << "Tera que comprar uma fita de tamanho " << sum << "." << endl;
    pontos.clear();
  }
