  ...
  freopen("output.txt", "w", stdout);
    ...
    while (cin >> n) {
		...
		for (int i = 0; i < n; i++) {
			...
			points.push_back(Point2D<double>(x, y));
		}
 
		int ans = 2;
		vector<Point2D<double>> centers;
 
		for (int i = 0; i < n; i++) {
			for (int j = i + 1; j < n; j++) {
				for (int k = j + 1; k < n; k++) {
					Point2D<double> a = points[i];
					Point2D<double> b = points[j];
					Point2D<double> c = points[k];
 
					if (collinear(a, b, c) == false) {
						double r = ccRadius(a, b, c);
						Point2D<double> mid = ccCenter(a, b, c);
						int cnt = 0; 
 
						for (int p = 0; p < n; p++) {
							if (compareFloats(points[p].distanceToPoint(mid), r) == 0) {
								cnt++;
							}
						}
