...
int main(void){
  ...
  while (cin >> n) {
    ...
    for (int i = 0; i < n; i++) {
      ...
      points.push_back(Point2D<double>(a, b));
    }
    ...
    for (int i = 0; i < n; i++) {   
      for (int j = i + 1; j < n; j++) {
        double l = 0.0, r = 0.0;
        Point2D<double> s = points[i];
        Point2D<double> e = points[j];

        for (int k = 0; k < n; k++) {
          if (points[k].sideOf(s, e) == 1) {
            l += abs(points[k].distanceToLine(s, e));
          } else {
            r += abs(points[k].distanceToLine(s, e));
          }
        }
